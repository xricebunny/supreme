/// PriceOracle — stores a rolling history of BTC/USDT prices indexed by block height.
...
access(all) contract PriceOracle {
    ...
    access(all) resource Admin {
        ...
        access(all) fun pushPrice(price: UFix64, timestamp: UFix64) {
            ...
            if PriceOracle.latestBlock > 0 {
                if let lastEntry = PriceOracle.priceHistory[PriceOracle.latestBlock] {
                    ...
                    let maxChange = lastPrice * UFix64(PriceOracle.maxChangeBps) / 10000.0
            ...
            let isNewEntry = PriceOracle.priceHistory[blockHeight] == nil
            PriceOracle.priceHistory[blockHeight] = entry
            PriceOracle.latestBlock = blockHeight
            if isNewEntry {
        ...
        access(all) fun pruneBeforeBlock(beforeBlock: UInt64) {
            ...
            let keys = PriceOracle.priceHistory.keys
            for key in keys {
                if key < beforeBlock {
                    PriceOracle.priceHistory.remove(key: key)
                    removedCount = removedCount + 1
                }
            }
            PriceOracle.entryCount = PriceOracle.entryCount - removedCount
