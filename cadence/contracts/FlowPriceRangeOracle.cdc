/// FlowPriceRangeOracle — companion to FlowPriceOracle that stores high/low ranges per push interval.
...
access(all) contract FlowPriceRangeOracle {
    ...
    access(all) resource Admin {
        ...
        access(all) fun pushRange(high: UFix64, low: UFix64) {
            ...
            FlowPriceRangeOracle.ranges[blockHeight] = range
        ...
        access(all) fun pruneBeforeBlock(beforeBlock: UInt64) {
            let keys = FlowPriceRangeOracle.ranges.keys
            for key in keys {
                if key < beforeBlock {
                    FlowPriceRangeOracle.ranges.remove(key: key)
                }
            }
        }
    }
    ...
    init() {
        self.ranges = {}
        self.AdminStoragePath = /storage/flowPriceRangeOracleAdmin
